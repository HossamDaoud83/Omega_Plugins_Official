...
test('plugin manifest is omega-str', () => {
  const m = JSON.parse(fs.readFileSync(path.join(PLUGIN_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.strictEqual(m.name, 'omega-str');
  assert.ok(m.version);
});

test('required commands present', () => {
  const cmds = ["market","competitive","financial-model","ops-assess","org-design","ma-dd","planning","cost-opt"];
  for (const c of cmds) {
...
test('required agents present', () => {
  const agents = ["strategy","financial-advisory","operations","risk-compliance","data-analytics"];
