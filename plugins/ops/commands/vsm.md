---
description: Build current-state and future-state VSM for a target process.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
---

# /omega-ops:vsm

**Value Stream Map.** Build current-state and future-state VSM for a target process.

## When to use

When the engagement scope explicitly calls for value stream map work in the operational excellence domain. Look for keywords like: lean, six sigma, VSM, kaizen, DMAIC.
...
5. Place the deliverable in `05_Deliverables_Final/` per the Omega file-location standard. Filename pattern: `ops-vsm-<client>-<YYYYMMDD>.md`.
