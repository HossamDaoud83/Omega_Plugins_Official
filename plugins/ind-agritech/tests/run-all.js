#!/usr/bin/env node
// Smoke tests for omega-ind-agritech — manifest integrity, command count, skill presence.
...
test('plugin.json is valid', () => {
  ...
  assert.strictEqual(manifest.name, 'omega-ind-agritech');
...
test('skill SKILL.md exists', () => {
  const skillFile = path.join(PLUGIN_ROOT, 'skills', 'agritech', 'SKILL.md');
  assert.ok(fs.existsSync(skillFile), `missing ${skillFile}`);
});

test('quality-standards rule exists', () => {
  const rule = path.join(PLUGIN_ROOT, 'rules', 'agritech-quality-standards.md');
