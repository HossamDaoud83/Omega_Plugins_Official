...
test('plugin manifest is omega-hlt', () => {
  const m = JSON.parse(fs.readFileSync(path.join(PLUGIN_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.strictEqual(m.name, 'omega-hlt');
  assert.ok(m.version);
});

test('required commands present', () => {
  const cmds = ["workflow","his-req","hl7-fhir","hipaa-gap","patient-journey","interop","clinical-kpis","telehealth"];
  for (const c of cmds) {
