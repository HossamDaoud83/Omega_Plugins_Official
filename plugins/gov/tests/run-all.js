...
test('plugin manifest is omega-gov', () => {
  const m = JSON.parse(fs.readFileSync(path.join(PLUGIN_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.strictEqual(m.name, 'omega-gov');
  assert.ok(m.version);
});

test('required commands present', () => {
  const cmds = ["egov-assess","citizen-svc","policy","transparency","508","procurement","stakeholder","ria"];
  for (const c of cmds) {
