#!/usr/bin/env node
// Smoke tests for omega-esg — manifest integrity, command count, skill presence.
...
test('plugin.json is valid', () => {
  ...
  assert.strictEqual(manifest.name, 'omega-esg');
...
test('skill SKILL.md exists', () => {
  const skillFile = path.join(PLUGIN_ROOT, 'skills', 'esg', 'SKILL.md');
  assert.ok(fs.existsSync(skillFile), `missing ${skillFile}`);
});

test('quality-standards rule exists', () => {
  const rule = path.join(PLUGIN_ROOT, 'rules', 'esg-quality-standards.md');
