#!/usr/bin/env node
// Smoke tests for omega-ai-contracts — manifest integrity, command count, skill presence.
...
test('plugin.json is valid', () => {
  ...
  assert.strictEqual(manifest.name, 'omega-ai-contracts');
...
test('skill SKILL.md exists', () => {
  const skillFile = path.join(PLUGIN_ROOT, 'skills', 'aicontracts', 'SKILL.md');
  assert.ok(fs.existsSync(skillFile), `missing ${skillFile}`);
});

test('quality-standards rule exists', () => {
  const rule = path.join(PLUGIN_ROOT, 'rules', 'aicontracts-quality-standards.md');
