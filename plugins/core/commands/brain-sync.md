...
2. Write sanitized copy to `/mnt/d/Obsidian Notes Taken/01_Instincts_Aggregated/`
...
1. Verify central brain exists at `/mnt/d/Obsidian Notes Taken/`. If not, run `node tools/seed-central-brain.js` once.
2. For each pending instinct:
   ```bash
   node plugins/core/scripts/obsidian/sanitizer.js \
     --in .brain/01_Instincts/INS-YYYY-NNN-slug.md \
     --out "/mnt/d/Obsidian Notes Taken/01_Instincts_Aggregated/INS-YYYY-NNN-slug.md"
   ```
...
```bash
grep -ri "<your client name>" "/mnt/d/Obsidian Notes Taken/01_Instincts_Aggregated/"
```
