...
test('plugin manifest is omega-esi', () => {
  const m = JSON.parse(fs.readFileSync(path.join(PLUGIN_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.strictEqual(m.name, 'omega-esi');
  assert.ok(m.version);
});

test('required commands present', () => {
  const cmds = ["assess","api-review","arch-review","migration","data-map","patterns","vendor-eval","cutover"];
  for (const c of cmds) {
...
test('required skills present', () => {
  const skills = ["esi","enterprise-integration"];
...
test('required agents present', () => {
  const agents = ["enterprise-integration"];
