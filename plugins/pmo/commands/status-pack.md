---
description: Author weekly/monthly status pack from tracker state.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
---

# /omega-pmo:status-pack

**Status Pack.** Author weekly/monthly status pack from tracker state.

## When to use

When the engagement scope explicitly calls for status pack work in the pmo & program delivery domain. Look for keywords like: PMO, portfolio, program, project, RAID.
...
5. Place the deliverable in `05_Deliverables_Final/` per the Omega file-location standard. Filename pattern: `pmo-status-pack-<client>-<YYYYMMDD>.md`.
