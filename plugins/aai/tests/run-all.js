...
test('plugin manifest is omega-aai', () => {
  const m = JSON.parse(fs.readFileSync(path.join(PLUGIN_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.strictEqual(m.name, 'omega-aai');
  assert.ok(m.version);
});

test('required commands present', () => {
  const cmds = ["assess","process-mine","rpa-feasibility","agent-design","roi","bot-inventory","hyperauto","guardrails"];
  for (const c of cmds) {
